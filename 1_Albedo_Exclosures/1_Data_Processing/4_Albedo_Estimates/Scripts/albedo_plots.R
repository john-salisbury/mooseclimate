        ...
        #PLOT 1A - Composite albedo on one plot (stacked) --------------
                ...
                #Build complex graph using cowplot package
                        ...
                        g1 <- ggplot(data = subset(albedo_reg, Group == "Composite" & Region == "Trøndelag"), aes(x = Years_Since_Exclosure, y = Mean_Subplot_Albedo, color = Treatment)) +
                                geom_ribbon(aes(ymin = (Mean_Subplot_Albedo - SE), ymax = (Mean_Subplot_Albedo + SE), fill = Treatment), alpha = 0.35, lwd = 0.1) +
                                geom_point(aes(shape = Treatment), size = 0.6) +
        ...
        #COMPOSITE ALBEDO -----------
                ...
                #COMPLEX PLOT  -------------
                        ...
                        g1 <- ggplot(subset(albedo_means, Group == "Composite" & Season == "Winter"), aes(x = Years_Since_Exclosure, y = Mean_Subplot_Albedo, color = Treatment, group = Treatment)) +
                                ...
                                geom_point(size = 0.4) +
                                theme_bw() +
                                facet_grid(~Month, labeller = labeller(Month = c("Jan", "Feb", "Mar"))) +
                                labs(x = "Years Since Exclosure", y = "Albedo") +
                                scale_x_continuous(breaks = c(2,4,6,8,10)) +
