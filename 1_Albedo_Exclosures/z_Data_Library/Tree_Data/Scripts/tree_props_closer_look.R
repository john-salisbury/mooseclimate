...
for(i in 1:length(site_codes)){
        ...
        for(j in min_year:max_year){
                ...
                for(k in 1:length(subs)){
                        ...
                        decid_prop <- sum(temp$Quantity[temp$Plot == subs[k] & temp$Taxa != "Picea abies (Gran)" & temp$Taxa != "Juniperus communis (Einer)" & temp$Taxa != "Pinus sylvestris (Furu)" & temp$Taxa != "Betula pendula (Lavlandbjørk)" & temp$Taxa != "Betula pubescens (Bjørk)"]) / temp$Total_trees_in_subplot[temp$Plot == subs[k]][1]
                        
                        
                        birch_prop_1 <- sum(temp$Quantity[temp$Plot == subs[k] & temp$Taxa == "Betula pubescens (Bjørk)"]) / temp$Total_trees_in_subplot[temp$Plot == subs[k]][1]
                        birch_prop_2 <- sum(temp$Quantity[temp$Plot == subs[k] & temp$Taxa == "Betula pendula (Lavlandbjørk)"]) / temp$Total_trees_in_subplot[temp$Plot == subs[k]][1]
                        birch_prop <- birch_prop_1 + birch_prop_2
                        #Create temp row
                        row <- data.frame("LocalityName" = local,
                                          ...
                                          "Prop_pine" = pine_prop,
                                          "Prop_birch" = birch_prop,
                                          "Prop_decid" = decid_prop)
...
final$Prop_decid <- as.numeric(final$Prop_decid)
...
write.csv(final, file = '1_Albedo_Exclosures/z_Data_Library/Tree_Data/Usable_Data/tree_props_closer_look.csv', row.names = TRUE)
...
final <- read.csv(file = '1_Albedo_Exclosures/z_Data_Library/Tree_Data/Usable_Data/tree_props_closer_look.csv', header = TRUE)
...
#Multiply each row x4 (once for each prop)
final <- final[rep(seq_len(nrow(final)), each = 4), ]
...
for(i in 1:length(used_sites)){
        ...
        for(j in min_yr:max_yr){
                ...
                for(k in 1:length(plots)){
                        ...
                        #Add Deciduous prop & label
                        final$Species[final$LocalityCode == site & final$Years_Since_Exclosure == j & final$Plot == plot][3] <- "Deciduous"
                        final$Prop[final$LocalityCode == site & final$Years_Since_Exclosure == j & final$Plot == plot][3] <- final$Prop_decid[final$LocalityCode == site & final$Years_Since_Exclosure == j & final$Plot == plot][3]
                        
                        #Add Birch prop & label
                        final$Species[final$LocalityCode == site & final$Years_Since_Exclosure == j & final$Plot == plot][4] <- "Birch"
                        final$Prop[final$LocalityCode == site & final$Years_Since_Exclosure == j & final$Plot == plot][4] <- final$Prop_birch[final$LocalityCode == site & final$Years_Since_Exclosure == j & final$Plot == plot][4]
                }
...
ggplot(data = reg_prop, aes(x = Years_Since_Exclosure, y = Mean_Prop, color = interaction(Treatment, Species), linetype = interaction(Treatment, Species))) +
        ...
        ggplot(data = reg_prop[reg_prop$Species == "Birch",], aes(x = Years_Since_Exclosure, y = Mean_Prop, color = Treatment)) +
                geom_line() +
                facet_wrap(~Counties)
